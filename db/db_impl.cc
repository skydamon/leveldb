...
namespace leveldb
{
  ...
  struct DBImpl::CompactionState
  {
    ...
    explicit CompactionState(Compaction* c) : compaction(c), smallest_snapshot(0), outfile(nullptr),
  ...
  DBImpl::DBImpl(const Options& raw_options, const std::string& dbname) : env_(raw_options.env),
  ...
  namespace
  {
    ...
    struct IterState
    {
      ...
      IterState(port::Mutex* mutex, MemTable* mem, MemTable* imm, Version* version) : mu(mutex),
                                                                                      version(
                                                                                        version),
                                                                                      mem(mem),
                                                                                      imm(imm){}
  ...
  Iterator* DBImpl::NewIterator(const ReadOptions& options)
  {
    ...
    return NewDBIterator(this, user_comparator(), iter, (options.snapshot !=
                                                         nullptr ? static_cast<const SnapshotImpl*>(options.snapshot)->sequence_number() : latest_snapshot),
                         seed);
  }
